...
$(document).ready(function(){
	...
	*/
	...
	if($mobileMenu.length){
		...
		let isExcludeMobileSwipe;
		isExcludeMobileSwipe = false;
		if ($('.js_exclude_mobile_swipe').length) {
			isExcludeMobileSwipe = true;
		}
	...
	$(document).on('click', '.ajax_load_btn_pagination', function(){
		var $this = $(this),
			...
			if (url) {
				...
				url = $this.siblings('.wrap_pagination').find('.pagination .next a').attr('href');
				var bCsType = true;
			...
			$.ajax({
				...
				success: function(html){
					...
					if (bCsType) {
						...
						$this.siblings('.wrap_pagination').html('');
						$this.siblings('.wrap_pagination').append(htmlPagination);
						
						
						var htmlContent = $(html).find('.items').html();
	...
	$('body').on( 'click', '.refresh', function(e){
		...
		$.ajax({
			...
			if($('.captcha_sid').length)
			{
				...
				$('.captcha_img').attr('src', 'bitrix/tools/captchaa7eb.jpg?captcha_sid=' + text);
			}
		});
	});
	...
	if(arPriorityOptions['THEME']['PRINT_BUTTON'] == 'Y' && typeof notShowPrintIcon == 'undefined')
	{
		...
		if(!th.hasClass('loading'))
		{
			...
			if('clear_cache' in objUrl)
			{
				if(objUrl.clear_cache == 'Y')
					add_url = '%26clear_cache%3dY.html';
			}
